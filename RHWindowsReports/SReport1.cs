...
{
    public static class SReport1
    {
        public static void ProcessReport()
        {
            DateTime sdt;
            string headerBackColor = ConfigurationManager.AppSettings["headerBackColor"];
            string bDays = ConfigurationManager.AppSettings["BackDays"];
            if (bDays != "")
            { sdt = DateTime.Now.AddDays(-(Convert.ToInt32(bDays))); }
            else
             sdt = DateTime.Now.AddDays(-1); //-5
            ...
            /* DataTable dt = DBHelper.GetDataTableFromDB("GetSReport1", sdt1, Edt1, "10", ConfigurationManager.AppSettings["ConnectionString"]);
             string TableContant = GetHtmlTable(dt);
             ...
             DataTable distinctValues = view.ToTable(true, "ManagerEmail");
             if (ConfigurationManager.AppSettings["IsTest"] == "true")
             {
                 EmailHelper.SendEmail(subject, Ourcontant, "sonia25sep@gmail.com");
                 EmailHelper.SendEmail(subject, Ourcontant, "johar@writeme.com");
             }
             else
             {
                 foreach (DataRow dRowE in distinctValues.Rows)
                 {
                     if (dRowE["ManagerEmail"].ToString() != "")
                     {
                         EmailHelper.SendEmail(subject, Ourcontant, dRowE["ManagerEmail"].ToString());
                         EmailHelper.SendEmail(subject, Ourcontant, "sonia25sep@gmail.com");
                     }
                 }
             }*/
          ...
          //  foreach (DataRow dRow in distinctDeptName.Rows)
            ...
            //    if (dRow["DeptName"].ToString() != "")
             ...
             //       DataTable dtDepatrment = dt.Select("DeptName='" + departmentName + "'").CopyToDataTable();
                    string subject = "Summary -" +" All Department Report"+" for " + System.DateTime.Now;
                   // TableContant = GetHtmlTable(viewDept);
                    string Ourcontant = " <span style = \"font-family:Arial;font-size:10pt\"><br />Summary report for complaints registered between <b>" + sdt1 + "</b> and <b>" + Edt1 + "</b>.  <br /><br />" + TableContant + "<br /><br /><span style='color:" + headerBackColor + "'><b>Thanks & Regards,<br/>Complaint Department</span></b><br/></span>";
                   ...
                   // DataView view = new DataView(dtDepatrment);
                    ...
                    if (ConfigurationManager.AppSettings["IsTest"] == "true")
                    {
                        ...
                        EmailHelper.SendEmail(subject, Ourcontant, "idrissawerwala@gmail.com;johar@writeme.com;yehia.khafaja@royalehayat.com;y.khafaja@gmail.com");//sonia25sep@gmail.com
                       // EmailHelper.SendEmail(subject, Ourcontant, "johar@writeme.com,yehia.khafaja@royalehayat.com");
                       // EmailHelper.SendEmail(subject, Ourcontant, "yehia.khafaja@royalehayat.com");
                        countMail++;
                    }
                    else
                    {
                        foreach (DataRow dRowE in distinctValues.Rows)
                        {
                            ...
                            {
                                EmailHelper.SendEmail(subject, Ourcontant, dRowE["ManagerEmail"].ToString());
                                // EmailHelper.SendEmail(subject, Ourcontant, "sayzaf@gmail.com");//sonia25sep@gmail.com
                            }
                        }
                        countMail++;
                ...
                if (countMail > 0)
                { EmailHelper.SendEmail("RoyalHayat - Summary Report- " + countMail + " between " + sdt1 + " And " + Edt1, "Successfully sent " + countMail + " email " + "between " + sdt1 + " And " + Edt1, "idrissawerwala@gmail.com;johar@writeme.com;yehia.khafaja@royalehayat.com;y.khafaja@gmail.com"); }
                else { EmailHelper.SendEmail("RoyalHayat - Summary Report -: " + countMail, " or No email sent today :", "idrissawerwala@gmail.com;johar@writeme.com;yehia.khafaja@royalehayat.com;y.khafaja@gmail.com"); }
        ...
        private static string GetHtmlTable(DataTable dt)
        {
            ...
            try
            {
                ...
                html.Append("<tr>");
                //html.Append("<th style='border: " + tableBorder + "px solid black;border-collapse: collapse;background-color:" + headerBackColor + ";color:" + headerForeColor + ";font-size:14px;font-weight:bold;'>Complaint No</th>");
                html.Append("<th style='border: " + tableBorder + "px solid black;border-collapse: collapse;background-color:" + headerBackColor + ";color:" + headerForeColor + ";font-size:14px;font-weight:bold;'>Department</th>");
                ...
                foreach (DataRow dRow in dt.Rows)
                {
                    ...
                    string dat = dt1.ToString("dd-MM-yyyy HH:mm:ss"); //"HH:mm:ss"
                    string datMain = dtMain.ToString("dd-MM-yyyy HH:mm:ss");
                    string remark = "";
                    string mainRemarks = "";
                    if (oldcomplainno == complainno)
                    {
                       //sz remark = dRow["Remarks"].ToString();
                        remark = dRow["MainRemarks"].ToString();
                    }
                    else
                    {
                    ...
                    string reference = complainno + " " + "<br/><br/>"  + remark;
                    string Patientn = " " + Patient + " Mob:" + MobileNo;
