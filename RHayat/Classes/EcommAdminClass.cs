    ...
    public static class EcommAdminClass
    {
        ...
        public static DropDownList getdropdown(DropDownList drp, int TID, string switch1, string switch2, string switch3, string TableName)//haresh
        {
            ...
            else if (TableName == "ICTR_HD")
            {
                ...
                if (switch2 != "")
                {
                    int TRSM = Convert.ToInt32(switch2);
                    //drp.DataSource = DB.ICTR_HD.Where(p => p.TenentID == TID && p.transid == TRS && p.transsubid == TRSM && p.MYSYSNAME == switch3);
                    var Datas = from item in DB.ICTR_HD.Where(p => p.TenentID == TID && p.transid == TRS && p.transsubid == TRSM && p.MYSYSNAME == switch3).ToList()
                                select new
                                {
                ...
                else
                {
                    ...
                    var Datas = from item in DB.ICTR_HD.Where(p => p.TenentID == TID && p.transid == TRS && p.MYSYSNAME == switch3).ToList()
                                select new
                                {
                                    Name = item.InvoiceNO + " - " + ((Int64)item.MYTRANSID).ToString() + " - " + (Convert.ToDateTime(item.ENTRYDATE).ToString("dd/MM/yyyy")) + " - " + item.TransDocNo,
                                    ID = ((Int64)item.MYTRANSID).ToString()
        ...
        public static void insert_ICTR_DT(int TenentID, int MYTRANSID, int locationID, int MYID, int MyProdID, string REFTYPE, string REFSUBTYPE, string PERIOD_CODE, string MYSYSNAME, int JOID, int JOBORDERDTMYID, int ACTIVITYID, string DESCRIPTION, string UOM, decimal QUANTITY, decimal UNITPRICE, decimal AMOUNT, decimal OVERHEADAMOUNT, string BATCHNO, int BIN_ID, string BIN_TYPE, string GRNREF, decimal DISPER, decimal DISAMT, decimal TAXPER, decimal TAXAMT, decimal PROMOTIONAMT, int CRUP_ID, string GLPOST, string GLPOST1, string GLPOSTREF1, string GLPOSTREF, string ICPOST, string ICPOSTREF, DateTime EXPIRYDATE, bool ACTIVE, string SWITCH1, int COMPANYID1, int DelFlag, string ITEMID, string Status = null)
        {
        ...
        public static void insertICIT_BR_TMP(int TenentID, int MyProdID, string period_code, string MySysName, int UOM, int SIZECODE, int COLORID, int BinID, string BatchNo, string Serialize, int MYTRANSID, int LocationID, decimal NewQty, string Reference, string RecodName, DateTime ProdDate, DateTime ExpiryDate, DateTime LeadDays2Destroy, string Active, int CRUP_ID, bool MinusAllow = false)
        {
        ...
        public static List<ICTR_HD> grdPOICTR_HD(int TID, int transid, int transsubid)//parimal
        {
            var List = DB.ICTR_HD.Where(p => p.TenentID == TID && p.ACTIVE == true && (p.transid == transid && p.transsubid == transsubid)).ToList();
            return List;
        }

        public static List<ICTR_DT> getListICTR_DT(int TID, int MYTRANSID)//parimal
        {
        ...
        public static List<ICTR_HD> getListICTR_HD(int TID, int transid, int transsubid)//parimal
        {
            var List = DB.ICTR_HD.Where(p => p.transid == transid && p.transsubid == transsubid && p.TenentID == TID).ToList();
            return List;
        }
        ...
        public static void insertICITEMS_PRICE(int TenentID = 0, int MYPRODID = 0, int UOM = 0, string ITEMID = "", decimal ORIGINALPRICE = 0, int MAXDISCOUNT = 0, decimal SPECIALSALE = 0, string REFERENCE = "", long CRUP_ID = 0, string LOCATIONID = "", int COMPANYID = 0)
        {
        ...
        public static void insertICTR_DTEditForreopen(int MYTRANSID = 0, int MYID = 0, int MyProdID = 0, int TenentID = 0, int locationID = 0, decimal? Prieshtotal = 0, decimal? FInelOC = 0)
        {
            ...
            ICTR_DT objICTR_DT = DB.ICTR_DT.Single(p => p.MYTRANSID == MYTRANSID && p.MYID == MYID && p.MyProdID == MyProdID && p.TenentID == TenentID && p.locationID == locationID);
