...
{
    public partial class planmealsetupUOM : System.Web.UI.Page
    {
        ...
        {
            //Listview1.DataSource = DB.planmealsetup.Where(p => p.TenentID == TID && p.ACTIVE == true);
            //Listview1.DataBind();

            //var List1 = DB.planmealsetup.Where(p => p.TenentID == TID && p.ACTIVE == true).Select(p=>p.planid).Distinct().ToList();
            //List<Database.planmealsetup> TempListPlanMeal = new List<Database.planmealsetup>();            

            //foreach (var item in List1)
            //{
            //    var obj = DB.planmealsetup.FirstOrDefault(p => p.MealType == item);
            ...
            //List<Database.planmealsetup> TempListPlanMeal = new List<Database.planmealsetup>();
            //List<Database.planmealsetup> ListPlan = DB.planmealsetup.Where(p => p.TenentID == TID).ToList();
            //foreach (Database.planmealsetup item in ListPlan)
            //{
            //    List<Database.planmealsetup> list1 = DB.planmealsetup.Where(p => p.planid == item.planid).ToList();
            //    foreach (Database.planmealsetup item2 in list1)
            //    {
            //        List<Database.planmealsetup> obj = DB.planmealsetup.Where(p => p.planid == item2.planid && p.MealType == item2.MealType).ToList();
            //        if (obj.Count() > 0)
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            else if (btnAdd.Text == "Add")
            {
                ...
                if (DB.planmealsetups.Where(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == planid && p.MealType == MealType && p.MYPRODID == MYPRODID && p.UOM == UOM).Count() < 1)
                {
                    ...
                    objplanmealsetup.UOM = Convert.ToInt32(drpUOM.SelectedValue);
                ...
                {
                    Database.planmealsetup objplanmealsetup = DB.planmealsetups.Single(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == planid && p.MealType == MealType && p.MYPRODID == MYPRODID && p.UOM == UOM);
                    //Server Content Send data Yogesh
            ...
            else if (btnAdd.Text == "Update")
            {
                ...
                if (ViewState["PID"] != null && ViewState["Meal"] != null && ViewState["Prod"] != null && ViewState["UOM"] != null)
                {
                    int pid = Convert.ToInt32(ViewState["PID"]);
                    int meal = Convert.ToInt32(ViewState["Meal"]);
                    long prod = Convert.ToInt64(ViewState["Prod"]);
                    int UOM = Convert.ToInt32(ViewState["UOM"]);
                    Database.planmealsetup objplanmealsetup = DB.planmealsetups.Single(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == pid && p.MealType == meal && p.MYPRODID == prod && p.UOM == UOM);
                    //objplanmealsetup.planid = Convert.ToInt32(drpPlanID.SelectedValue);
                    //objplanmealsetup.MealType = Convert.ToInt32(drpMealID.SelectedValue);
                    //objplanmealsetup.MYPRODID = Convert.ToInt64(drpProduct.SelectedValue);
        ...
        {
            List<Database.tblProduct_Plan> Listplan = DB.tblProduct_Plan.Where(p => p.TenentID == TID && p.active == true).ToList();

            drpPlanID.DataSource = Listplan.OrderBy(p => p.planname1);
            drpPlanID.DataTextField = "planname1";
            drpPlanID.DataValueField = "planid";
            drpPlanID.DataBind();
            drpPlanID.Items.Insert(0, new ListItem("-- Select --", "0"));

            drpMealID.DataSource = DB.REFTABLEs.Where(p => p.TenentID == TID && p.REFTYPE == "Food" && p.REFSUBTYPE == "MealType" && p.ACTIVE == "Y").OrderBy(p => p.SWITCH1);
            ...
            drpUOM.Items.Insert(0, new ListItem("-- Select --", "0"));

            drpFromPlan.DataSource = DB.tblProduct_Plan.Where(p => p.TenentID == TID && p.active == true).OrderBy(p => p.planname1);
            ...
            drpTOPlan.DataSource = DB.tblProduct_Plan.Where(p => p.TenentID == TID && p.active == true).OrderBy(p => p.planname1);
        ...
        //    int Totalrec = DB.planmealsetup.Count();
        ...
        //    ((AcmMaster)Page.Master).BindList(Listview1, (DB.planmealsetup.Where(p => p.TenentID == TID).OrderBy(m => m.planid).Take(take).Skip(Skip)).ToList());
        //    ViewState["Take"] = take;
        //    ViewState["Skip"] = Skip;
        ...
        protected void Listview1_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
                ...
                if (Request.QueryString["AD"] != null)
                {
                    ...
                    if (Admin == UID)
                    {
                        if (DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.planid == PID && p.MealType == Meal && p.ExpectedDelDate != null).Count() > 0)
                        {
                            ...
                            {
                                Database.planmealsetup obj = DB.planmealsetups.Single(p => p.TenentID == TID && p.MYPRODID == item.MYPRODID && p.UOM == item.UOM && p.planid == PID && p.MealType == Meal && p.ACTIVE == true);
                                obj.ACTIVE = false;
                                DB.SaveChanges();
            ...
            if (e.CommandName == "btnEdit")
            {
                ...
                //Database.planmealsetup objplanmealsetup = DB.planmealsetup.Single(p => p.TenentID == TID && p.planid == PID && p.MealType == Meal);
        ...
        protected void Listview3_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            if (e.CommandName == "btnDelete11")
            {
                ...
                int Meal = Convert.ToInt32(ID[1]);
                long Prod = Convert.ToInt64(ID[2]);
                int UOM = Convert.ToInt32(ID[3]);

                Database.planmealsetup objplanmealsetup = DB.planmealsetups.Single(p => p.TenentID == TID && p.planid == PID && p.MealType == Meal && p.MYPRODID == Prod && p.UOM == UOM);
            ...
            if (e.CommandName == "btnEdit11")
            {
                ...
                drpMealID.SelectedValue = objplanmealsetup.MealType.ToString();
                drpProduct.SelectedValue = objplanmealsetup.MYPRODID.ToString();
                int MYPRODID = Convert.ToInt32(objplanmealsetup.MYPRODID);
                BindUOM(MYPRODID);
                drpUOM.SelectedValue = objplanmealsetup.UOM.ToString();
        ...
        protected void btnsearch_Click(object sender, EventArgs e)
        {
            ...
            //List<Database.planmealsetup> List = DB.planmealsetup.Where(p => p.TenentID == TID && p.LOCATION_ID==LID && p.planid == pid && p.MealType == MEAL && p.ACTIVE==false).ToList();
        ...
        protected void lnkbtnCopyplan_Click(object sender, EventArgs e)
        {
            ...
            if (Fromplan != TOplan)
            {
                ...
                {
                    if (DB.planmealsetups.Where(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == TOplan && p.MealType == items.MealType && p.MYPRODID == items.MYPRODID && p.UOM == items.UOM).Count() < 1)
                    {
        ...
        protected void drpProduct_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                int MYPRODID = Convert.ToInt32(drpProduct.SelectedValue);
                BindUOM(MYPRODID);
            }
        }

        public void BindUOM(int MYPRODID)
        {
            List<Database.ICUOM> ListMultiuom = new List<Database.ICUOM>();
            List<Database.ICUOM> ListTemp = new List<Database.ICUOM>();

            TBLPRODUCT objTBLPRODUCT = DB.TBLPRODUCTs.Single(p => p.MYPRODID == MYPRODID && p.TenentID == TID && p.LOCATION_ID == LID);
            int UOM = Convert.ToInt32(objTBLPRODUCT.UOM);

            Database.ICUOM listuom = DB.ICUOMs.Single(p => p.TenentID == TID && p.UOM == UOM);
            ListTemp.Add(listuom);

            List<Database.Tbl_Multi_Color_Size_Mst> listmultics = DB.Tbl_Multi_Color_Size_Mst.Where(p => p.TenentID == TID && p.CompniyAndContactID == MYPRODID && p.RecordType == "MultiUOM").ToList();
        ...
        //        ((AcmMaster)Page.Master).BindList(Listview1, (DB.planmealsetup.Where(p => p.TenentID == TID).OrderBy(m => m.planid).Take(Tvalue).Skip(Svalue)).ToList());
        //        ChoiceID = ID;
        //        //((AcmMaster)Page.Master).GetCurrentNavigation(ChoiceID, Showdata, ListView2);
