...
{
    public partial class HBContract : System.Web.UI.Page
    {
        ...
        public void deleteduplicate()
        {
            ...
            {
                List<Database.planmealcustinvoice> ListInvoice1 = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == items.MYTRANSID && p.MYPRODID == items.MYPRODID && p.UOM == items.UOM && p.DeliveryTime == items.DeliveryTime && p.DeliveryMeal == items.DeliveryMeal && p.DayNumber == items.DayNumber && p.ExpectedDelDate == items.ExpectedDelDate).ToList();
        ...
        public void SwitchoONOFF(string switch1)
        {
            ...
            if (switch1 == "editOff")
            {
                ...
                if (ViewState["MYTRANSID"] != null)
                {
                    ...
                    int customerID = Convert.ToInt32(drpCustomer.SelectedValue);

                    int planid = Convert.ToInt32(drpPlan.SelectedValue);

                    int Count = customermealcount(customerID, planid);

                    if (Count > 0)
                    {
                        int MEalC = MealCount(MYTRANSID);
                        if (MEalC == Count)
                        { btnCopyWeekWise.Enabled = true; }
                        else
                        { btnCopyWeekWise.Enabled = false; }
        ...
        public int MealCount(int MYTRANSID)
        {
            int count = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID).GroupBy(p => p.DeliveryMeal).Count();
            return count;
        }

        public int customermealcount(int customerID, int planid)
        {
            List<Database.tblcontact_addon1_dtl> ListDtl = DB.tblcontact_addon1_dtl.Where(p => p.TenentID == TID && p.customerID == customerID && p.planid == planid && p.LikeType == "M").ToList();
            return ListDtl.Count();
        }
        ...
        public void adddt(int Mytrans, Label lbldelday, Label lblFDate, Label lblDayName, Label lbldeldayH, Label lblProd, TextBox txtproduct, DropDownList drpUOM, TextBox txtWeight, TextBox txtCalories, TextBox txtProtain, TextBox txtCarbs, TextBox txtFat, TextBox txtsaleprice, int DeliverySequence, TextBox txtqtyNO)
        {
            pnlErrorMsg.Visible = false;
            lblerrorMsg.Text = "";
            ...
            List<Database.planmealcustinvoice> ListExist = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == Mytrans && p.MYPRODID == MYPRODID && p.UOM == UOM && p.DeliveryMeal == DeliveryMeal && p.DayNumber == DayNumber).ToList();
            if (ListExist.Count() < 1)
            {
                ...
                List<Database.planmealsetup> ListPlanpro = DB.planmealsetups.Where(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == plan && p.MealType == DeliveryMeal && p.MYPRODID == MYPRODID && p.UOM == UOM).ToList();

                if (ListPlanpro.Count() > 0)
                {
                    Database.planmealsetup objpan = ListPlanpro.Single(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == plan && p.MealType == DeliveryMeal && p.MYPRODID == MYPRODID && p.UOM == UOM);
                ...
                objplanmealcustinvoice.UOM = UOM;
                ...
                objplanmealcustinvoice.UploadDate = DateTime.Now;
                objplanmealcustinvoice.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                objplanmealcustinvoice.SynID = 1;
        ...
        public void CheckAddon1(int CustomerId, int DeliveryCountry, int planid)
        {
            if (DB.tblcontact_addon1.Where(p => p.TenentID == TID && p.CustomerId == CustomerId && p.LocationID == LID).Count() < 1)
            {
                if (DB.TBLCOMPANYSETUPs.Where(p => p.TenentID == TID && p.COMPID == CustomerId).Count() > 0)
                {
                    DB.TBLCOMPANYSETUPs.Single(p => p.TenentID == TID && p.COMPID == CustomerId);
                }
                ...
                objtblcontact_addon1.CustomerId = CustomerId;
                ...
                {
                    objtblcontact_addon1.DeliveryCountry = DeliveryCountry.ToString();
                }
                else
                {
                    objtblcontact_addon1.DeliveryCountry = "126";
                }
                objtblcontact_addon1.AdvCollectionStatus = "3";// (drpAdvCollectionStatus.SelectedValue);
                objtblcontact_addon1.PromotionOn = "3";// (drpPromotionOn.SelectedValue);
                objtblcontact_addon1.agreedcabs = "1500";// txtAgreedCabs.Text; 
            ...
            List<Database.tblcontact_addon1_dtl> ListDTLmeal = DB.tblcontact_addon1_dtl.Where(p => p.TenentID == TID && p.LocationID == LID && p.customerID == CustomerId && p.planid == planid).ToList();

            string DeliveryTime = ListDTLmeal.Count() > 0 ? ListDTLmeal.FirstOrDefault().DeliveryTime : "89415";

            List<Database.planmealsetup> ListPLansetup = DB.planmealsetups.Where(p => p.TenentID == TID && p.planid == planid).GroupBy(p => p.MealType).Select(p => p.FirstOrDefault()).ToList();

            foreach (Database.planmealsetup items in ListPLansetup)
            {
                if (DB.tblcontact_addon1_dtl.Where(p => p.TenentID == TID && p.LocationID == LID && p.customerID == CustomerId && p.planid == planid && p.DeliveryTime == DeliveryTime && p.ItemCode == items.MealType).Count() < 1)
                {
                    ...
                    objtblcontact_addon1_dtl.customerID = CustomerId;// Convert.ToInt32(drpCustomerId.SelectedValue);
                    objtblcontact_addon1_dtl.myID = DB.tblcontact_addon1_dtl.Where(p => p.TenentID == TID && p.LocationID == LID && p.customerID == CustomerId).Count() > 0 ? DB.tblcontact_addon1_dtl.Where(p => p.TenentID == TID && p.LocationID == LID && p.customerID == CustomerId).Max(p => p.myID) + 1 : 1;
                    objtblcontact_addon1_dtl.LikeType = "M";
                    objtblcontact_addon1_dtl.planid = planid;
                    objtblcontact_addon1_dtl.DeliveryTime = "89415";
                    objtblcontact_addon1_dtl.ItemCode = items.MealType;
                    objtblcontact_addon1_dtl.DateAdded = DateTime.Now;
                    objtblcontact_addon1_dtl.UploadDate = DateTime.Now;
                    //objtblcontact_addon1_dtl.Uploadby = UID;
                    objtblcontact_addon1_dtl.SynID = 1;
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (ViewState["TEmpDeleteRecord"] != null && ViewState["MYTRANSID"] != null)
            {
                DeleteRecord = (List<Database.planmealcustinvoice>)ViewState["TEmpDeleteRecord"];

                int MYTRANSID = Convert.ToInt32(ViewState["MYTRANSID"]);

                int SelectedWeek = Convert.ToInt32(DrpWeek.SelectedValue);

                if (DeleteRecord.Count() > 0)
                {
                    foreach (Database.planmealcustinvoice items in DeleteRecord)
                    {
                        string Str = "delete from planmealcustinvoice where TenentID=" + TID + " and MYTRANSID=" + MYTRANSID + " AND planid ='" + planid + "' AND DeliveryMeal = '" + MealType + "' AND NoOfWeek = '" + SelectedWeek + "' AND MYPRODID ='" + items.MYPRODID + "' AND UOM = '" + items.UOM + "' ";
                        command2 = new SqlCommand(Str, con);
                        con.Open();
                        command2.ExecuteReader();
                        con.Close();
                ...
                ViewState["TEmpDeleteRecord"] = null;
            }

            List<Database.planmealsetup> List = new List<Database.planmealsetup>();
            ...
            for (int i = 0; i < ListView2.Items.Count; i++)
            {
                ...
                int UOM = Convert.ToInt32(drpUOM.SelectedValue);
                string uomName = drpUOM.SelectedItem.ToString();

                if (ListPlanmeal.Where(p => p.MYPRODID == Prod && p.UOM == UOM).Count() > 0)
                {
                    Database.planmealsetup obj = ListPlanmeal.Single(p => p.MYPRODID == Prod && p.UOM == UOM);
                    List.Add(obj);
                }

                if (List.Where(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == planid && p.MealType == MealType && p.MYPRODID == Prod && p.UOM == UOM).Count() > 1)
                {
                    dropMSG += "<B><U>" + PRodName + "</U></B>  and   <b><U> " + uomName + " </U></B>  are already Exist in List Please Change UOM in Product List. <BR>";
                }
            ...
            if (ListPlan.Count() > 0)
            {
                for (int i = 0; i < ListView2.Items.Count; i++)
                {
                    ...
                    if (RDODay1.Checked == true && lblDate1.Text != "")
                    {
                        SelectCount1++;
                    }

                    if (RDODay2.Checked == true && lblDate2.Text != "")
                    {
                        SelectCount2++;
                    }

                    if (RDODay3.Checked == true && lblDate3.Text != "")
                    {
                        SelectCount3++;
                    }

                    if (RDODay4.Checked == true && lblDate4.Text != "")
                    {
                        SelectCount4++;
                    }

                    if (RDODay5.Checked == true && lblDate5.Text != "")
                    {
                        SelectCount5++;
                    }

                    if (RDODay6.Checked == true && lblDate6.Text != "")
                    {
                        SelectCount6++;
                    }
                }
                ...
                if (SelectCount1 == 0 && lblDate1.Text != "")
                {
                    MSG += "<B><U>" + Planname + "</U></B>  For  <b><U> " + meal + " </U></B>  Add at least 1 Item in  " + lblDate1.Text + " . <BR>";
                }

                if (SelectCount2 == 0 && lblDate2.Text != "")
                {
                    MSG += "<B><U>" + Planname + "</U></B>  For  <b><U> " + meal + " </U></B>  Add at least 1 Item in  " + lblDate2.Text + " .  <BR>";
                }

                if (SelectCount3 == 0 && lblDate3.Text != "")
                {
                    MSG += "<B><U>" + Planname + "</U></B>  For  <b><U> " + meal + " </U></B>  Add at least 1 Item in " + lblDate3.Text + " .  <BR>";
                }

                if (SelectCount4 == 0 && lblDate4.Text != "")
                {
                    MSG += "<B><U>" + Planname + "</U></B>  For  <b><U> " + meal + " </U></B>  Add at least 1 Item in  " + lblDate4.Text + " . <BR>";
                }

                if (SelectCount5 == 0 && lblDate5.Text != "")
                {
                    MSG += "<B><U>" + Planname + "</U></B>  For  <b><U> " + meal + " </U></B>  Add at least 1 Item in  " + lblDate5.Text + " . <BR>";
                }
                if (SelectCount6 == 0 && lblDate6.Text != "")
                {
                    MSG += "<B><U>" + Planname + "</U></B>  For  <b><U> " + meal + " </U></B>  Add at least 1 Item in  " + lblDate6.Text + " .  <BR>";
                }
            ...
            if (btnAdd.Text == "Add New")
            {
                ...
                List<Database.TBLCOMPANYSETUP> ListCustomer = DB.TBLCOMPANYSETUPs.Where(p => p.TenentID == TID && p.CompanyType == CompanyType && p.Active == "Y").ToList();
            ...
            {
                if (drpExpecteddriver.SelectedValue == "0")
                {
                    pnlErrorMsg1.Visible = true;
                    lblerrorMsg1.Text = "Select Any Driver";
                    return;
                }
                ...
                if (DB.planmealcustinvoiceHDs.Where(p => p.TenentID == TID && p.MYTRANSID == Mytrans).Count() == 0)
                {
                    ...
                    OBJHD.UploadDate = DateTime.Now;
                    OBJHD.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                    OBJHD.SynID = 1;
                ...
                for (int i = 0; i < ListView2.Items.Count; i++)
                {
                    ...
                    if (RDODay1.Checked == true)
                    {
                        adddt(Mytrans, lbldelday1, lblFDate1, lblDayName1, lbldelday11, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty1);
                        DeliverySequence++;
                    }
                    if (RDODay2.Checked == true)
                    {
                        adddt(Mytrans, lbldelday2, lblFDate2, lblDayName2, lbldelday21, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty2);
                        DeliverySequence++;
                    }
                    if (RDODay3.Checked == true)
                    {
                        adddt(Mytrans, lbldelday3, lblFDate3, lblDayName3, lbldelday31, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty3);
                        DeliverySequence++;
                    }
                    if (RDODay4.Checked == true)
                    {
                        adddt(Mytrans, lbldelday4, lblFDate4, lblDayName4, lbldelday41, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty4);
                        DeliverySequence++;
                    }
                    if (RDODay5.Checked == true)
                    {
                        adddt(Mytrans, lbldelday5, lblFDate5, lblDayName5, lbldelday51, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty5);
                        DeliverySequence++;
                    }
                    if (RDODay6.Checked == true)
                    {
                        adddt(Mytrans, lbldelday6, lblFDate6, lblDayName6, lbldelday61, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty6);
                        DeliverySequence++;
                    }
                }
                    ...
                    for (int i = 0; i < ListView2.Items.Count; i++)
                    {
                        ...
                        foreach (Database.planmealcustinvoice items in Listplanmealcustinvoice)
                        {
                            if (Listplanmealcustinvoice.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.DeliveryID == items.DeliveryID && p.MYPRODID == items.MYPRODID && p.ActualDelDate == null).Count() > 0)
                            {
                                ...
                                objeditprod.UploadDate = DateTime.Now;
                                objeditprod.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                                objeditprod.SynID = 2;
                        ...
                        List<Database.planmealcustinvoice> ListInvoiceEdit = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.DeliveryMeal == DeleveryMeal && p.NoOfWeek == Week).ToList();


                        if (ListInvoiceEdit.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM).Count() > 0)
                        {
                            ...
                            foreach (string Val in days1)
                            {
                                if (RDODay1.Checked == true && Val == "Sat")
                                {
                                    ...
                                    if (ListInvoiceEdit.Where(p => p.OprationDay == 1).Count() > 0)
                                    {
                                        ...
                                        {
                                            Database.planmealcustinvoice objinvoice1 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 1 && p.NoOfWeek == Week && p.DayNumber == Daynum && p.ExpectedDelDate == Exp);
                                            ...
                                            objinvoice1.UploadDate = DateTime.Now;
                                            objinvoice1.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                                            objinvoice1.SynID = 2;
                                    ...
                                    else
                                    {
                                        ...
                                        if (lbldate1 <= EndDate)
                                        {
                                            adddt(MYTRANSID, lbldelday1, lblFDate1, lblDayName1, lbldelday11, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty1);
                                            DeliverySequence++;
                                        }
                                ...
                                else
                                {
                                    if (Val == "Sat")
                                    {
                                        if (ListInvoiceEdit.Where(p => p.OprationDay == 1).Count() > 0)
                                        {
                                            Database.planmealcustinvoice objinvoicedel1 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 1 && p.NoOfWeek == Week);
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                ...
                                if (RDODay2.Checked == true && Val == "Sun")
                                {
                                    ...
                                    if (ListInvoiceEdit.Where(p => p.OprationDay == 2).Count() > 0)
                                    {
                                        ...
                                        {
                                            Database.planmealcustinvoice objinvoice2 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 2 && p.NoOfWeek == Week && p.DayNumber == Daynum && p.ExpectedDelDate == Exp);
                                            ...
                                            objinvoice2.UploadDate = DateTime.Now;
                                            objinvoice2.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                                            objinvoice2.SynID = 2;
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            DB.SaveChanges();
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                    ...
                                    else
                                    {
                                        ...
                                        if (lbldate2 <= EndDate)
                                        {
                                            adddt(MYTRANSID, lbldelday2, lblFDate2, lblDayName2, lbldelday21, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty2);
                                            DeliverySequence++;
                                        }
                                    }
                                }
                                else
                                {
                                    if (Val == "Sun")
                                    {
                                        if (ListInvoiceEdit.Where(p => p.OprationDay == 2).Count() > 0)
                                        {
                                            Database.planmealcustinvoice objinvoicedel2 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 2 && p.NoOfWeek == Week);
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            //DB.planmealcustinvoices.DeleteObject(objinvoice);
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                ...
                                if (RDODay3.Checked == true && Val == "Mon")
                                {
                                    ...
                                    if (ListInvoiceEdit.Where(p => p.OprationDay == 3).Count() > 0)
                                    {
                                        ...
                                        {
                                            Database.planmealcustinvoice objinvoice3 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 3 && p.NoOfWeek == Week && p.DayNumber == Daynum && p.ExpectedDelDate == Exp);
                                            ...
                                            objinvoice3.UploadDate = DateTime.Now;
                                            objinvoice3.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                                            objinvoice3.SynID = 2;
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            DB.SaveChanges();
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                    ...
                                    else
                                    {
                                        ...
                                        if (lbldate3 <= EndDate)
                                        {
                                            adddt(MYTRANSID, lbldelday3, lblFDate3, lblDayName3, lbldelday31, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty3);
                                            DeliverySequence++;
                                        }
                                    }
                                }
                                else
                                {
                                    if (Val == "Mon")
                                    {
                                        if (ListInvoiceEdit.Where(p => p.OprationDay == 3).Count() > 0)
                                        {
                                            Database.planmealcustinvoice objinvoicedel3 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 3 && p.NoOfWeek == Week);
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            //DB.planmealcustinvoices.DeleteObject(objinvoice);
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                ...
                                if (RDODay4.Checked == true && Val == "Tue")
                                {
                                    ...
                                    if (ListInvoiceEdit.Where(p => p.OprationDay == 4).Count() > 0)
                                    {
                                        ...
                                        {
                                            Database.planmealcustinvoice objinvoice4 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 4 && p.NoOfWeek == Week && p.DayNumber == Daynum && p.ExpectedDelDate == Exp);
                                            ...
                                            objinvoice4.UploadDate = DateTime.Now;
                                            objinvoice4.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                                            objinvoice4.SynID = 2;
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            DB.SaveChanges();
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                    ...
                                    else
                                    {
                                        ...
                                        if (lbldate4 <= EndDate)
                                        {
                                            adddt(MYTRANSID, lbldelday4, lblFDate4, lblDayName4, lbldelday41, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty4);
                                            DeliverySequence++;
                                        }
                                    }
                                }
                                else
                                {
                                    if (Val == "Tue")
                                    {
                                        if (ListInvoiceEdit.Where(p => p.OprationDay == 4).Count() > 0)
                                        {
                                            Database.planmealcustinvoice objinvoicedel4 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 4 && p.NoOfWeek == Week);
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            //DB.planmealcustinvoices.DeleteObject(objinvoice);
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                ...
                                if (RDODay5.Checked == true && Val == "Wed")
                                {
                                    ...
                                    if (ListInvoiceEdit.Where(p => p.OprationDay == 5).Count() > 0)
                                    {
                                        ...
                                        {
                                            Database.planmealcustinvoice objinvoice5 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 5 && p.NoOfWeek == Week && p.DayNumber == Daynum && p.ExpectedDelDate == Exp);
                                            ...
                                            objinvoice5.UploadDate = DateTime.Now;
                                            objinvoice5.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                                            objinvoice5.SynID = 2;
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            DB.SaveChanges();
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                    ...
                                    else
                                    {
                                        ...
                                        if (lbldate5 <= EndDate)
                                        {
                                            adddt(MYTRANSID, lbldelday5, lblFDate5, lblDayName5, lbldelday51, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty5);
                                            DeliverySequence++;
                                        }
                                    }
                                }
                                else
                                {
                                    if (Val == "Wed")
                                    {
                                        if (ListInvoiceEdit.Where(p => p.OprationDay == 5).Count() > 0)
                                        {
                                            Database.planmealcustinvoice objinvoicedel5 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 5 && p.NoOfWeek == Week);
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            //DB.planmealcustinvoices.DeleteObject(objinvoice);
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                ...
                                if (RDODay6.Checked == true && Val == "Thu")
                                {
                                    ...
                                    if (ListInvoiceEdit.Where(p => p.OprationDay == 6).Count() > 0)
                                    {
                                        ...
                                        {
                                            Database.planmealcustinvoice objinvoice6 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 6 && p.NoOfWeek == Week && p.DayNumber == Daynum && p.ExpectedDelDate == Exp);
                                            ...
                                            objinvoice6.UploadDate = DateTime.Now;
                                            objinvoice6.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                                            objinvoice6.SynID = 2;
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            DB.SaveChanges();
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
                                    ...
                                    else
                                    {
                                        ...
                                        if (lbldate6 <= EndDate)
                                        {
                                            adddt(MYTRANSID, lbldelday6, lblFDate6, lblDayName6, lbldelday61, lblProd, txtproduct, drpUOM, txtWeight, txtCalories, txtProtain, txtCarbs, txtFat, txtsaleprice, DeliverySequence, txtqty6);
                                            DeliverySequence++;
                                        }
                                    }
                                }
                                else
                                {
                                    if (Val == "Thu")
                                    {
                                        if (ListInvoiceEdit.Where(p => p.OprationDay == 6).Count() > 0)
                                        {
                                            Database.planmealcustinvoice objinvoicedel6 = ListInvoiceEdit.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.OprationDay == 6 && p.NoOfWeek == Week);
                                            //crupID = Convert.ToInt32(objinvoice.CRUP_ID);
                                            //DB.planmealcustinvoices.DeleteObject(objinvoice);
                                            //string Logdelete = " MYTRANSID=" + MYTRANSID + ", CustomerID=" + CID + ", planid=" + planid + ", DeliveryMeal=" + Meal + ", MYPRODID=" + PID;
        ...
        protected void btnCancel_Click(object sender, EventArgs e)
        {
            ...
            switch1 = "AddOff";
            Response.Redirect("/Master/HBContract.aspx");
        }
        ...
        protected void ListViewContract_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "btnCopyContract")
            {
                ...
                if (ListCUstHD.Count() > 0)
                {
                    ...
                    foreach (Database.planmealcustinvoice items in Listinvoice)
                    {
                        List<Database.planmealcustinvoice> ListExist = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == NewMYTRANSID && p.DeliveryMeal == items.DeliveryMeal && p.MYPRODID == items.MYPRODID && p.UOM == items.UOM && p.DayNumber == items.DayNumber).ToList();
                        if (ListExist.Count() < 1)
                        {
                            ...
                            objinvice.DisplayWeek = items.DisplayWeek;
                            ...
                            objinvice.UploadDate = DateTime.Now;
                            objinvice.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                            objinvice.SynID = 1;
        ...
        public void Editinvoice(int CID, int MYTRANSID, int Plan, int Meal)
        {
            ...
            if (MYTRANSID != 0)
            {
                ...
                if (objplanHD.DefaultDriverID != null && objplanHD.DefaultDriverID.ToString() != "99999")
                    drpExpecteddriver.SelectedValue = objplanHD.DefaultDriverID.ToString();
                else
                    drpExpecteddriver.SelectedValue = "0";
                ...
                if (ListInvo.Count() > 0)
                {
                    ...
                    txtTotalWeek.Text = ListInvo.Where(p => p.ExpectedDelDate >= TONow).Count() > 0 ? ListInvo.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.planid == Plan && p.DeliveryMeal == Meal && p.ExpectedDelDate >= TONow).FirstOrDefault().TotalWeek.ToString() : ListInvo.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.planid == Plan && p.DeliveryMeal == Meal).FirstOrDefault().TotalWeek.ToString();
        ...
        protected void drpDeliveryMeal_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                pnlErrorMsg.Visible = true;
                lblerrorMsg.Text = "please enter Contract Date First";
                return;
            }
        ...
        public void GetProductList()
        {
            ...
            int Week = 1;
            if (DrpWeek.Items.Count > 0)
            {
                Week = Convert.ToInt32(DrpWeek.SelectedValue);
            }
            

            GetLISTData(planid, MID, Week);
        ...
        public void getproductEdit(int MYTRANSID)
        {
            ...
            if (ListInvoice.Count() > 0)
            {
                ...
                for (int i = 0; i < ListView2.Items.Count; i++)
                {
                    ...
                    Label lblSetUOM = (Label)ListView2.Items[i].FindControl("lblSetUOM");

                    int PID = Convert.ToInt32(lblProd.Text);
                    int UOM = Convert.ToInt32(lblUOM.Text);

                    if (ListInvoice.Where(p => p.MYPRODID == PID && p.UOM == UOM).Count() > 0)//&& p.CustomerID == CID && p.NoOfWeek == Week
                    {
                        //Parimal
                        Database.planmealcustinvoice objinvoice = ListInvoice.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM).FirstOrDefault();//&& p.CustomerID == CID && p.NoOfWeek == Week 

                        drpUOM.SelectedValue = UOM.ToString();
                        lblSetUOM.Text = UOM.ToString();
                        ...
                        List<Database.planmealsetup> ListPlan = DB.planmealsetups.Where(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == Planid && p.MealType == MID && p.MYPRODID == PID && p.UOM == UOM).ToList();
                        if (ListPlan.Count() > 0)
                        {
                            Database.planmealsetup objplan = ListPlan.Single(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == Planid && p.MealType == MID && p.MYPRODID == PID && p.UOM == UOM);
                        ...
                        List<Database.planmealcustinvoice> ListInvoice123 = ListInvoice.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM).ToList();//&& p.CustomerID == CID && p.NoOfWeek == Week

                        foreach (Database.planmealcustinvoice items in ListInvoice123)
                        {
                            Database.planmealcustinvoice objinvoice12 = ListInvoice.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == PID && p.UOM == UOM && p.DayNumber == items.DayNumber && p.DeliveryMeal == MID).FirstOrDefault();//&& p.CustomerID == CID  && p.NoOfWeek == Week
                ...
                for (int i = 0; i < ListView2.Items.Count; i++)
                {
                    ...
                    if (List.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.ActualDelDate != null).Count() > 0)
                    {
                        updatedate = Today.AddDays(1);
                    }
        ...
        public void GetLISTData(int planid, int MID, int Week)
        {
            if (ViewState["MYTRANSID"] != null)
            {
                int MYTRANSID = Convert.ToInt32(ViewState["MYTRANSID"]);

                List<Database.planmealcustinvoice> ListInvoice = new List<Database.planmealcustinvoice>();

                if (Week != 1)
                {
                    ListInvoice = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.planid == planid && p.DeliveryMeal == MID && p.NoOfWeek == Week).ToList();
                }
                else
                {
                    ListInvoice = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.planid == planid && p.DeliveryMeal == MID && p.DisplayWeek == Week).ToList();
                }

                if (ListInvoice.Count() > 0)
                {
                    GetLIstDataProductWithTransation(planid, MID, Week, MYTRANSID);
                }
                else
                {
                    GetLIstDataProduct(planid, MID, Week);
                }
        ...
        public void GetLIstDataProduct(int planid, int MID, int Week)
        {
            List<Database.TBLPRODUCT> ListTBLPRODUCT = DB.TBLPRODUCTs.Where(p => p.TenentID == TID).ToList();
            List<Database.planmealsetup> Listplanmealsetup = DB.planmealsetups.Where(p => p.ACTIVE == true && p.TenentID == TID).ToList();
            List<Database.planmealsetup> ListProduct = new List<Database.planmealsetup>();
            ...
            {
                if (ListTBLPRODUCT.Where(p => p.TenentID == TID && p.MYPRODID == item3.MYPRODID).Count() > 0)
                {
                    ListProduct.Add(item3);
                }
            }

            TempPlanmealSetUp = ListProduct;
            ViewState["TempPlanmealSetUp"] = TempPlanmealSetUp;

            ListView2.DataSource = ListProduct;// Listpro.GroupBy(p => p.MYPRODID).Select(p => p.FirstOrDefault()).OrderBy(p => p.serial_no);// ListTBLPRODUCT;
            ListView2.DataBind();
            ...
            foreach (Database.planmealsetup item3 in Listplanmealsetup.Where(a => a.planid == planid && a.MealType == MID))//.GroupBy(p => p.MYPRODID).Select(p => p.FirstOrDefault()))
            {
                if (TempPlanmealSetUp.Where(p => p.TenentID == TID && p.planid == planid && p.MealType == MID && p.MYPRODID == item3.MYPRODID && p.UOM == item3.UOM).Count() < 1)
                {
                    if (ListTBLPRODUCT.Where(p => p.TenentID == TID && p.MYPRODID == item3.MYPRODID).Count() > 0)
                    {
                        Listpro.Add(item3);
            ...
            ListView5.DataSource = Listpro.OrderBy(p => p.serial_no);
            ListView5.DataBind();
        }

        public void GetLIstDataProductWithTransation(int planid, int MID, int Week, int MYTRANSID)
        {
            List<Database.TBLPRODUCT> ListTBLPRODUCT = DB.TBLPRODUCTs.Where(p => p.TenentID == TID).ToList();
            List<Database.planmealcustinvoice> ListInvoice = new List<Database.planmealcustinvoice>();
            ...
            List<Database.planmealsetup> Listplanmealsetup = DB.planmealsetups.Where(p => p.ACTIVE == true && p.TenentID == TID && p.planid == planid && p.MealType == MID).ToList();

            List<Database.planmealsetup> ListProduct = new List<Database.planmealsetup>();

            foreach (Database.planmealsetup item3 in Listplanmealsetup)
            {
                foreach (Database.planmealcustinvoice item in ListInvoice.Where(p => p.MYPRODID == item3.MYPRODID).GroupBy(p => p.UOM).Select(p => p.FirstOrDefault()))
                {
                    if (item.MYPRODID == item3.MYPRODID && item.UOM == item3.UOM)
                    {
                        if (ListTBLPRODUCT.Where(p => p.TenentID == TID && p.MYPRODID == item3.MYPRODID).Count() > 0)
                        {
                            ListProduct.Add(item3);
            ...
            ListView2.DataSource = ListProduct;
            ListView2.DataBind();
            ...
            if (txtBeingDate.Text != "")
            {
                ...
                if (txtEndDate.Text != "")
                {
                    ...
                    if (ViewState["MYTRANSID"] != null)
                    {
                        ...
                        {
                            Classes.Toastr.ShowToast(Page, Classes.Toastr.ToastType.Info, "please Copy Full Plan if you complate First Week", "Success!", Classes.Toastr.ToastPosition.TopCenter);
                        }
        ...
        public void NoWeek(int MYTRANSID)
        {
            pnlErrorMsg.Visible = false;
            lblerrorMsg.Text = "";
            ...
            if (drpCustomer.SelectedValue != "0")
            {
                ...
                if (drpPlan.SelectedValue != "0")
                {
                    ...
                    else
                    {
                        if (MYTRANSID != 0)
                        {
                            List<Database.planmealcustinvoice> ListDT = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID).GroupBy(p => p.NoOfWeek).Select(p => p.FirstOrDefault()).ToList();
                            if (ListDT.Count() > 0)
                            {
        ...
        protected void drpweekofday_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (drpweekofday.SelectedValue != "0")
            {
                ...
                List<tempweekofdayCon> tempTable = new List<tempweekofdayCon>();
                foreach (string Val in days)
                {
                    tempweekofdayCon obj = new tempweekofdayCon();
                    if (Val == "Sat")
                    {
                        obj.Name = "1.Saturday";
                        obj.ID = 1;
            ...
            if (txtBeingDate.Text != "" && txtEndDate.Text != "" && ViewState["TimeCount"] != null)
            {
                ...
                else
                {
                    if (txtEndDate.Text != "")
                    {
                        ...
                        decimal EnterDay = Convert.ToDecimal((EDT - BDT).TotalDays);
        ...
        protected void drpCustomer_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (CID != 0)
            {
                ...
                List<Database.planmealcustinvoiceHD> ListHD = DB.planmealcustinvoiceHDs.Where(p => p.TenentID == TID && p.CustomerID == CID && p.EndDate >= Today).ToList();
        ...
        public void DeliveryTime()
        {
            ...
            List<Database.REFTABLE> ListDelevaryTime = new List<Database.REFTABLE>();
            //List<Database.tblcontact_addon1_dtl> OBJDelevaryTime = DB.tblcontact_addon1_dtl.Where(p => p.TenentID == TID && p.LikeType == "M" && p.customerID == CID && p.planid == Planid && p.active == true).GroupBy(p => p.DeliveryTime).Select(p => p.FirstOrDefault()).ToList();
            List<Database.REFTABLE> ListDelevary = DB.REFTABLEs.Where(p => p.TenentID == TID && p.REFTYPE == "Food" && p.REFSUBTYPE == "DeliveryTime" && p.REFNAME1 == "Evening Once in Day").ToList();

            foreach (Database.REFTABLE item in ListDelevary)
            {
                Database.REFTABLE obj = item;
                ListDelevaryTime.Add(obj);
            }
        ...
        public void DeloveryMeal()
        {
            ...
            List<Database.planmealsetup> ListPLanmeal = DB.planmealsetups.Where(p => p.TenentID == TID && p.planid == plan).GroupBy(p => p.MealType).Select(p => p.FirstOrDefault()).ToList();

            foreach (Database.planmealsetup item2 in ListPLanmeal)
            {
                if (DB.REFTABLEs.Where(p => p.TenentID == TID && p.REFID == item2.MealType).Count() > 0)
                {
                    Database.REFTABLE obj2 = DB.REFTABLEs.Single(p => p.TenentID == TID && p.REFID == item2.MealType);
            ...
            List<Database.planmealcustinvoice> ListInvo = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.CustomerID == CID && p.planid == plan && p.DeliveryTime == Deltime && p.EndDate >= today).GroupBy(p => p.MealType).Select(p => p.FirstOrDefault()).ToList();
        ...
        protected void btnAddmeal_Click(object sender, EventArgs e)
        {
            ...
            string Delevery = lblDeliveryTIMEID.Text;
            List<Database.REFTABLE> ListREFTABLE = new List<Database.REFTABLE>();
            List<Database.tblcontact_addon1_dtl> Listplanmealsetup = DB.tblcontact_addon1_dtl.Where(p => p.TenentID == TID && p.customerID == CID && p.planid == Planid && p.active == true).ToList();//&& p.DeliveryTime == Delevery
            foreach (Database.tblcontact_addon1_dtl item2 in Listplanmealsetup)
            {
                if (DB.REFTABLEs.Where(p => p.TenentID == TID && p.REFID == item2.ItemCode).Count() > 0)
                {
            ...
            int Deltime = Convert.ToInt32(lblDeliveryTIMEID.Text);
            List<Database.planmealcustinvoice> ListInvo = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.CustomerID == CID && p.planid == Planid && p.DeliveryTime == Deltime && p.EndDate >= today).GroupBy(p => p.MealType).Select(p => p.FirstOrDefault()).ToList();
        ...
        protected void ListView2_ItemDataBound(object sender, ListViewItemEventArgs e)
        {
            ...
            Label lblSetUOM = (Label)e.Item.FindControl("lblSetUOM");

            int Prod = Convert.ToInt32(lblProd.Text);
            int UOM = Convert.ToInt32(lblUOM.Text);

            List<Database.tblProduct_Plan> ListPaln = DB.tblProduct_Plan.Where(p => p.TenentID == TID && p.planid == planid).ToList();
            ...
            if (e.Item.ItemType == ListViewItemType.DataItem)
            {
                if (btnAdd.Text == "Save")
                {
                    if (DB.ICITEMS_PRICE.Where(p => p.TenentID == TID && p.MYPRODID == Prod && p.UOM == UOM).Count() > 0)
                    {
                        decimal SalePrice = Convert.ToDecimal(DB.ICITEMS_PRICE.Single(p => p.TenentID == TID && p.MYPRODID == Prod && p.UOM == UOM).msrp);
                        txtsaleprice.Text = SalePrice.ToString();
                    }
                    else
                    {
                    ...
                    List<Database.Tbl_Multi_Color_Size_Mst> listmultics = DB.Tbl_Multi_Color_Size_Mst.Where(p => p.TenentID == TID && p.CompniyAndContactID == Prod && p.RecordType == "MultiUOM").ToList();
                    ...
                    if (ListTemp.Count() > 0)
                    {
                        ...
                        if (ListPaln.Where(p => p.TenentID == TID && p.planid == planid && p.CustomAllow == true).Count() > 0)
                        {
                            drpUOM.Enabled = true;
                        }
                        else
                    ...
                    {
                        Database.ICUOM listuom = DB.ICUOMs.Single(p => p.TenentID == TID && p.UOM == UOM);
                        ListTemp.Add(listuom);
                    ...
                    string Carbs = "36";
                    string Fat = "9";
                    List<Database.planmealsetup> ListPlan = DB.planmealsetups.Where(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == planid && p.MealType == MID && p.MYPRODID == Prod && p.UOM == UOM).ToList();
                    if (ListPlan.Count() > 0)
                    {
                        Database.planmealsetup objplan = ListPlan.Single(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == planid && p.MealType == MID && p.MYPRODID == Prod && p.UOM == UOM);
                ...
                else
                {
                    ...
                    if (ViewState["MYTRANSID"] != null)
                    {
                        ...
                        List<Database.planmealcustinvoice> ListInvoice = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == Prod && p.UOM == UOM && p.planid == planid && p.MealType == MID).ToList();
                        if (ListInvoice.Count() < 1)
                        {
                            if (DB.ICITEMS_PRICE.Where(p => p.TenentID == TID && p.MYPRODID == Prod && p.UOM == UOM).Count() > 0)
                            {
                                decimal SalePrice = Convert.ToDecimal(DB.ICITEMS_PRICE.Single(p => p.TenentID == TID && p.MYPRODID == Prod && p.UOM == UOM).msrp);
        ...
        protected void btnCopyFullPlan_Click(object sender, EventArgs e)
        {
            ...
            if (ViewState["MYTRANSID"] != null)
            {
                ...
                if (MYTRANSID != 0)
                {
                    ...
                    Copyfullplan(MYTRANSID, Week, plan, 1);
                }
                else
                {
                    pnlErrorMsg.Visible = true;
        ...
        public void Copyfullplan(int MYTRANSID, int Week, int plan, int FromWeek)
        {
            int daycount = 0;
            string[] days1 = tags_1.Text.Split(',');
            ...
            if (Listinvoice.Count() > 0)
            {
                foreach (Database.planmealcustinvoice items in Listinvoice)
                {
                    Database.planmealcustinvoice objupdatainvoice = Listinvoice.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.DeliveryID == items.DeliveryID && p.MYPRODID == items.MYPRODID && p.UOM == items.UOM);
                    ...
                    objupdatainvoice.UploadDate = DateTime.Now;
                    objupdatainvoice.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                    objupdatainvoice.SynID = 2;
                    DB.SaveChanges();
                }

                int FromDeleteWeek = FromWeek + 1;
                List<Database.planmealcustinvoice> ListDelete = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.NoOfWeek >= FromDeleteWeek && p.DisplayWeek == null).ToList();

                if (ListDelete.Count() > 0)
                {
                    foreach (Database.planmealcustinvoice itemdelete in ListDelete)
                    {
                        Database.planmealcustinvoice objupdatainvoice = ListDelete.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.DeliveryID == itemdelete.DeliveryID && p.MYPRODID == itemdelete.MYPRODID && p.UOM == itemdelete.UOM);
                        DB.planmealcustinvoices.DeleteObject(objupdatainvoice);
                        DB.SaveChanges();
                    }

                    //string Str = "delete from planmealcustinvoice where TenentID=" + TID + " and MYTRANSID=" + MYTRANSID + "  AND NoOfWeek >= '" + FromDeleteWeek + "' and  DisplayWeek is null  ";
                    //command2 = new SqlCommand(Str, con);
                    //con.Open();
                    //command2.ExecuteReader();
                    //con.Close();
                ...
                int ToWeek = Week + 1;
                for (int i = FromWeek + 1; i <= ToWeek; i++)
                {
                    int ADDweek = i - FromWeek;

                    int dayst = daycount;

                    List<Database.planmealcustinvoice> Listweek = new List<Database.planmealcustinvoice>();

                    if (FromWeek == 1)
                    {
                        Listweek = Listinvoice.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.ACTIVE == true && p.DisplayWeek == 1).ToList();//&& p.CustomerID == CID
                    }
                    else
                    {
                        Listweek = Listinvoice.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.ACTIVE == true && p.NoOfWeek == FromWeek).ToList();//&& p.CustomerID == CID
                    }
                    ...
                    if (Listweek.Count() > 0)
                    {
                        foreach (Database.planmealcustinvoice items in Listweek)
                        {
                            ...
                            if (EndDate >= Fexp)
                            {
                                ...
                                int Final = fday + day;
                                List<Database.planmealcustinvoice> ListExist = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.MYPRODID == items.MYPRODID && p.UOM == items.UOM && p.DeliveryMeal == items.DeliveryMeal && p.DayNumber == Final).ToList();
                                if (ListExist.Count() < 1)
                                {
                                    Database.planmealcustinvoice objinvice = new planmealcustinvoice();
                                    objinvice.TenentID = TID;
        ...
        public void DayAgoDelevery(int MYTRANSID)
        {
            ...
            if (ListInvoice.Count() > 0)
            {
                foreach (Database.planmealcustinvoice items in ListInvoice)
                {
                    Database.planmealcustinvoice Objinvoice = ListInvoice.Single(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.DeliveryID == items.DeliveryID && p.MYPRODID == items.MYPRODID && p.UOM == items.UOM);
                    ...
                    Objinvoice.UploadDate = DateTime.Now;
                    Objinvoice.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                    Objinvoice.SynID = 2;
                    DB.SaveChanges();
                }
            }
        ...
        public void updatenoofweek(int MYTRANSID)
        {
            ...
            foreach (Database.planmealcustinvoice items in ListInvoice)
            {
                ...
                objdt.UploadDate = DateTime.Now;
                objdt.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                objdt.SynID = 2;
                DB.SaveChanges();
            }
        }
        ...
        public void AddNewDT(int MYTRANSID, int planid, int CustomerID, long MYPRODID, int UOM, string ProdName1, int DayNumber, int OprationDay, string WeekofDay, int TotalWeek, string NameOfDay, int No_OfWeek, int TotalDeliveryDay, int ActualDeliveryDay, int ExpectedDeliveryDay, int DeliveryTime, int DeliveryMeal, int DriverID, DateTime StartDate, DateTime EndDate, DateTime ExpectedDelDate, decimal Calories, decimal Carbs, decimal Protein, decimal Fat, decimal ItemWeight, int Qty, decimal Item_cost, decimal Item_price, decimal Total_price, string ShortRemark)
        {
            ...
            if (ListInvice.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.DeliveryMeal == DeliveryMeal && p.MYPRODID == MYPRODID && p.UOM == UOM && (p.ExpectedDelDate.Value.Day == Day && p.ExpectedDelDate.Value.Month == Month && p.ExpectedDelDate.Value.Year == Year) && p.ACTIVE == true).Count() < 1)
            {
                Database.planmealcustinvoice objinvice = new planmealcustinvoice();
                objinvice.TenentID = TID;
                objinvice.MYTRANSID = MYTRANSID;
            ...
            if (txtDriverChangeDate.Text != "")
            {
                ...
                if (ViewState["MYTRANSID"] != null)
                {
                    ...
                    if (Listhd.Count() > 0)
                    {
                        ...
                        objinv.UploadDate = DateTime.Now;
                        objinv.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                        objinv.SynID = 2;
                        DB.SaveChanges();
                    }
            ...
            if (txtUnHoldDate.Text != "")
            {
                ...
                if (ViewState["MYTRANSID"] != null)
                {
                    ...
                    if (DB.planmealcustinvoiceHDs.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID).Count() > 0)
                    {
                        ...
                        int Year = HoldDate.Year;
                        int Month = HoldDate.Month;
                        int Day = HoldDate.Day;

                        int FromWeek = Convert.ToInt32(List.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.ExpectedDelDate.Value.Year == Year && p.ExpectedDelDate.Value.Month == Month && p.ExpectedDelDate.Value.Day == Day).FirstOrDefault().NoOfWeek);
                        ...
                        calculate();
                        int plan = Convert.ToInt32(drpPlan.SelectedValue);
                        Copyfullplan(MYTRANSID, week, plan, FromWeek);
                        recalculate();
                        int Custo = Convert.ToInt32(drpCustomer.SelectedValue);
                        int Meal = Convert.ToInt32(drpDeliveryMeal.SelectedValue);
                        Editinvoice(Custo, MYTRANSID, plan, Meal);
        ...
        protected void lnkbtndriveradd_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ObjCompny.MYPRODID = 0;

                ObjCompny.Reload = false;
                ObjCompny.UploadDate = DateTime.Now;
                ObjCompny.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                ObjCompny.SynID = 1;
                ...
                ObjEmp.UploadDate = DateTime.Now;
                ObjEmp.Uploadby = ((USER_MST)Session["USER"]).FIRST_NAME;
                ObjEmp.SynID = 1;
        ...
        List<Database.planmealsetup> TempPlanmealSetUp = new List<Database.planmealsetup>();
        protected void ListView5_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            if (e.CommandName == "AddItem")
            {
                ...
                int UOM = Convert.ToInt32(ID[1]);

                List<Database.planmealsetup> List = DB.planmealsetups.Where(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == planID && p.MealType == MealType && p.MYPRODID == MYPRODID && p.UOM == UOM).ToList();

                if (List.Count > 0)
                {
                    TempPlanmealSetUp = (List<Database.planmealsetup>)ViewState["TempPlanmealSetUp"];

                    Database.planmealsetup objPlan = DB.planmealsetups.Single(p => p.TenentID == TID && p.LOCATION_ID == LID && p.planid == planID && p.MealType == MealType && p.MYPRODID == MYPRODID && p.UOM == UOM);

                    TempPlanmealSetUp.Add(objPlan);
                    ListView2.DataSource = TempPlanmealSetUp.OrderBy(p => p.serial_no);
                    ListView2.DataBind();

                    List<Database.planmealsetup> Listpro = new List<Database.planmealsetup>();
                    List<Database.planmealsetup> Listplanmealsetup = DB.planmealsetups.Where(p => p.ACTIVE == true && p.TenentID == TID).ToList();

                    foreach (Database.planmealsetup item3 in Listplanmealsetup.Where(a => a.planid == planID && a.MealType == MealType))//.GroupBy(p => p.MYPRODID).Select(p => p.FirstOrDefault()))
                    {
                        if (TempPlanmealSetUp.Where(p => p.TenentID == TID && p.planid == planID && p.MealType == MealType && p.MYPRODID == item3.MYPRODID && p.UOM == item3.UOM).Count() < 1)
                        {
                            if (DB.TBLPRODUCTs.Where(p => p.TenentID == TID && p.MYPRODID == item3.MYPRODID).Count() > 0)
                            {
                                Listpro.Add(item3);
        ...
        public string GetUOM(int UOM)
        {
            ...
            {
                string UOMNAME = DB.ICUOMs.Single(p => p.TenentID == TID && p.UOM == UOM).UOMNAME1;
                return UOMNAME;
            }
            else
            {
        ...
        protected void btnCopyWeekWise_Click(object sender, EventArgs e)
        {
            ...
            if (ViewState["MYTRANSID"] != null)
            {
                ...
                int plan = Convert.ToInt32(drpPlan.SelectedValue);
                int Week = Convert.ToInt32(txtTotalWeek.Text);
                int FromWeek = Convert.ToInt32(DrpWeek.SelectedValue);
        ...
        List<Database.planmealcustinvoice> DeleteRecord = new List<Database.planmealcustinvoice>();
        ...
        protected void drpUOM_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ViewState["MYTRANSID"] != null)
            {
                ...
                Label lblSetUOM = (Label)item.FindControl("lblSetUOM");

                int PID = Convert.ToInt32(lblProd.Text);
                int ChangeUOM = Convert.ToInt32(lblSetUOM.Text);

                int Planid = Convert.ToInt32(drpPlan.SelectedValue);
                int MID = drpDeliveryMeal.SelectedValue != "" ? Convert.ToInt32(drpDeliveryMeal.SelectedValue) : 0;
                int Week = Convert.ToInt32(DrpWeek.SelectedValue);

                if (ViewState["TEmpDeleteRecord"] != null)
                {
                    DeleteRecord = (List<Database.planmealcustinvoice>)ViewState["TEmpDeleteRecord"];
                }

                List<Database.planmealcustinvoice> ListInvoice = DB.planmealcustinvoices.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.planid == Planid && p.DeliveryMeal == MID && p.MYPRODID == PID && p.UOM == ChangeUOM && p.NoOfWeek == Week).ToList();

                if (ListInvoice.Count() > 0)
                {
                    foreach (Database.planmealcustinvoice items in ListInvoice)
                    {
                        if (DeleteRecord.Where(p => p.TenentID == TID && p.MYTRANSID == MYTRANSID && p.planid == Planid && p.DeliveryMeal == MID && p.MYPRODID == PID && p.UOM == ChangeUOM && p.NoOfWeek == Week).Count() < 1)
                        {
                            DeleteRecord.Add(items);
                        }
                    }
                }

                ViewState["TEmpDeleteRecord"] = DeleteRecord;
