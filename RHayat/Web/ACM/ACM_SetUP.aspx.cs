...
namespace Web.ACM
{
    public partial class ACM_SetUP : System.Web.UI.Page
    {
        ...
        public void bindsys(int PrivilageFor, ListView CommonListview)
        {
            ...
            int TID = Convert.ToInt32(drpTenet.SelectedValue);
            int PRIVILEGE_ID = Convert.ToInt32(drpPrevilegeMST.SelectedValue);
            var PrivilageID = DB.PRIVILEGE_MST.OrderBy(p => p.MODULE_ID).FirstOrDefault(p => p.PRIVILEGE_ID == PRIVILEGE_ID && p.TenentID == TID);
            int MODULE_ID = Convert.ToInt32(PrivilageID.MODULE_ID);
            ...
            if (PrivilageFor == 1)
                PRIVILEGE_MENU_ID = Convert.ToInt32(drproleMST.SelectedValue);
            ...
            FinalList = DB.PRIVILAGE_MENUDemon.Where(p => p.PRIVILEGE_MENU_ID == PRIVILEGE_MENU_ID && p.PRIVILEGE_ID == PRIVILEGE_ID && p.TenentID == TID && p.ACTIVE_FLAG == "Y" && p.PRIVILAGEFOR == PrivilageFor).ToList();

            if (List.Count() != FinalList.Count())
            {
                List<Database.FUNCTION_MST> ListDelete = DB.FUNCTION_MST.Where(p => p.MODULE_ID == MODULE_ID && p.ACTIVE_FLAG == 0).OrderBy(p => p.MENU_ORDER).OrderBy(p => p.MENU_NAME1).ToList();
                foreach (Database.FUNCTION_MST item in ListDelete)
                {
                    if (DB.PRIVILAGE_MENUDemon.Where(p => p.PRIVILEGE_MENU_ID == PRIVILEGE_MENU_ID && p.PRIVILEGE_ID == PRIVILEGE_ID && p.TenentID == TID && p.PRIVILAGEFOR == PrivilageFor && p.MENU_ID == item.MENU_ID).Count() > 0)
                    {
                        Database.PRIVILAGE_MENUDemon Delobj = DB.PRIVILAGE_MENUDemon.Single(p => p.PRIVILEGE_MENU_ID == PRIVILEGE_MENU_ID && p.PRIVILEGE_ID == PRIVILEGE_ID && p.TenentID == TID && p.PRIVILAGEFOR == PrivilageFor && p.MENU_ID == item.MENU_ID);
                        DB.PRIVILAGE_MENUDemon.DeleteObject(Delobj);
                        DB.SaveChanges();
                    }
                }
            }
            if (List.Count() != FinalList.Count())
            {
                foreach (Database.FUNCTION_MST item in List)
                {
                    if (FinalList.Where(p => p.MENU_ID == item.MENU_ID).Count() == 0)
                    {
                        ...
                        objPRIVILAGE_MENU.PRIVILEGE_MENU_ID = PRIVILEGE_MENU_ID;
                        ...
                        objPRIVILAGE_MENU.ActiveModule = "N";
            ...
            if (FinalList.Count() > 0)
            {
                if (TID == 15)
                {
                    if (FinalList.Where(p => p.MENU_LOCATION == "Separator" && (p.MENU_ID != 1000274 && p.MENU_ID != 1000700)).Count() > 0)// p.ACTIVETILLDATE >= DateTime.Now &&
                    {

                        CommonListview.DataSource = FinalList.Where(p => p.MENU_LOCATION == "Separator" && (p.MENU_ID != 1000274 && p.MENU_ID != 1000700));// p.ACTIVETILLDATE >= DateTime.Now &&
                        CommonListview.DataBind();
                        int MasterID = Convert.ToInt32(FinalList.First(p => p.MENU_LOCATION == "Separator").MASTER_ID);//p.ACTIVETILLDATE >= DateTime.Now && 
                ...
                {
                    if (FinalList.Where(p => p.MENU_LOCATION == "Separator").Count() > 0)// p.ACTIVETILLDATE >= DateTime.Now &&
                    {

                        CommonListview.DataSource = FinalList.Where(p => p.MENU_LOCATION == "Separator");// p.ACTIVETILLDATE >= DateTime.Now &&
                        CommonListview.DataBind();
                        int MasterID = Convert.ToInt32(FinalList.First(p => p.MENU_LOCATION == "Separator").MASTER_ID);//p.ACTIVETILLDATE >= DateTime.Now && 
        ...
        public void updatePM(ListView mainList, int PrivilageFor)
        {
            ...
            for (int i = 0; i < mainList.Items.Count; i++)
            {
                ...
                if (PrivilageFor == 3)
                {
                    string Activeuser = CHKUserActive.Checked == true ? "Y" : "N";
                    string userwisemodule = cheusewiseModule.Checked == true ? "Y" : "N";
                    List<Database.PRIVILAGE_MENUDemon> userwiseMOD = DB.PRIVILAGE_MENUDemon.Where(p => p.PRIVILAGEFOR == 2 && p.TenentID == TID && p.ActiveModule == "Y").ToList();
                    if (userwiseMOD.Count() > 0)
                    {
                        foreach (Database.PRIVILAGE_MENUDemon item in PRIList)
                        {
                            if (DB.PRIVILAGE_MENUDemon.Where(p => p.PRIVILEGE_MENU_ID == PRIVILEGE_MENUID && p.TenentID == TID && p.PRIVILAGEFOR == PrivilageFor && p.MENU_ID == item.MENU_ID).Count() == 1)
                            {
                                Database.PRIVILAGE_MENUDemon objuser = DB.PRIVILAGE_MENUDemon.Single(p => p.PRIVILEGE_MENU_ID == PRIVILEGE_MENUID && p.TenentID == TID && p.PRIVILAGEFOR == PrivilageFor && p.MENU_ID == item.MENU_ID);
                                objuser.Action = Activeuser;
                                if (DB.PRIVILAGE_MENUDemon.Where(p => p.TenentID == TID && p.PRIVILEGE_MENU_ID == PRIVILEGE_MENUID && p.PRIVILEGE_ID == PRIVILEGE_ID).Count() > 0)
                                {
                                    objuser.ActiveModule = userwisemodule;
                                }
                                DB.SaveChanges();
                            }
                        }
        ...
        protected void drpTenet_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (TID == 130)
            {
                List<Database.MODULE_MAP> ListModule = DB.MODULE_MAP.Where(p => p.TenentID == TID && p.ACTIVE_FLAG == "Y" && p.PRIVILEGE_ID == 1029).GroupBy(p => p.PRIVILEGE_ID).Select(p => p.FirstOrDefault()).ToList();
                foreach (Database.MODULE_MAP item in ListModule)
                {
                    if (DB.PRIVILEGE_MST.Where(p => p.PRIVILEGE_ID == item.PRIVILEGE_ID && p.TenentID == TID && p.PRIVILEGE_ID == 1029).Count() > 0)
                    {
                        Database.PRIVILEGE_MST obj = DB.PRIVILEGE_MST.Single(p => p.PRIVILEGE_ID == item.PRIVILEGE_ID && p.TenentID == TID);
            ...
            else
            {
                ...
                foreach (Database.MODULE_MAP item in ListModule)
                {
                    if (DB.PRIVILEGE_MST.Where(p => p.PRIVILEGE_ID == item.PRIVILEGE_ID && p.TenentID == TID).Count() > 0)
                    {
                        Database.PRIVILEGE_MST obj = DB.PRIVILEGE_MST.Single(p => p.PRIVILEGE_ID == item.PRIVILEGE_ID && p.TenentID == TID);
                        PRIVILEGEList.Add(obj);
                    }
        ...
        protected void drpPrevilegeMST_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            lblforUser.Text = "User Wise Menu For " + modulefoleHeader;
        ...
        protected void drpUserMST_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            int MODULE_ID = DB.MODULE_MAP.Single(p => p.TenentID == tenent && p.UserID == userID && p.PRIVILEGE_ID == PRIVILEGE_ID).MODULE_ID;
            if (DB.MODULE_MAP.Where(p => p.TenentID == tenent && p.UserID == userID).Count() > 0)
            {
                if (DB.MODULE_MAP.Where(p => p.TenentID == tenent && p.UserID == userID && p.SP1Name == "DefaultSet").Count() == 0)
                {
                    Database.MODULE_MAP objMap = DB.MODULE_MAP.Single(p => p.TenentID == tenent && p.UserID == userID && p.MODULE_ID == MODULE_ID);
                    objMap.SP1Name = "DefaultSet";
                    DB.SaveChanges();
                }
            }
            else //(DB.MODULE_MAP.Where(p => p.TenantID == tenent && p.PRIVILEGE_ID == PRIVILEGE_ID && p.UserID == userID && p.MODULE_ID == MODULE_ID && p.SP1Name == "DefaultSet").Count() == 0)
            {
                Database.MODULE_MAP objMap = new Database.MODULE_MAP();
                objMap.TenentID = tenent;
                objMap.PRIVILEGE_ID = PRIVILEGE_ID;
                objMap.LOCATION_ID = 1;
                objMap.MODULE_ID = MODULE_ID;
